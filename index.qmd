...
# Loading the dataset
data <- read.csv("data/updated_data.csv")
print(data)
...
executive_data <- data %>%
  ...
  count(job_title, name = "Frequency") # Count frequency of each job title
...
Introduction: Analyzing salary trends across job categories and company sizes provides insights into how organizational scale impacts compensation. This analysis highlights variations in average salaries for different job categories and helps identify which roles are more lucrative in small, medium, or large companies.
...
Group data by work_setting and employee_residence to calculate the average salary and the number of employees (employee_count) for each combination.
...
Bubble size indicates employee count, and bubble color represents employee residence.
...
References
Kaggle. (2020–2023). Job Market Trends, Salary Data, Job Titles, and Company Sizes. Retrieved from https://www.kaggle.com/datasets/murilozangari/jobs-and-salaries-in-data-field-2024
